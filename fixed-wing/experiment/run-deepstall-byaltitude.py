	...
	while True:
		...
		if int(vehicle.channels['7']) > 1514 and isnot_deepstalled:
			...
			while True:
				...
				print(pitch_down_now_time - pitch_down_start_time)
				if pitch_down_now_time - pitch_down_start_time >= 5:
					print("time out pitch down")
					break
				if current_alttitude <= 20.5 and current_alttitude >= 3:
					print("Reached deepstall altitude.")
					break
				#if pitch_angle  >= -12:
				#	print("Elevator down")
				vehicle.channels.overrides['2'] = 1285
					
				# else:
				# 	vehicle.channels.overrides['2'] = 1545
		...
		now = time.time()
		time_ago = now -start
		if (vehicle.location.global_relative_frame.alt <= 1 and time_ago > 60*2) or (key == ord('q')):
...
def adjustElevator(trajectory_angle):
	if int(vehicle.channels['7']) > 1514:
		if trajectory_angle >= 52.5:
			vehicle.channels.overrides['2'] = 1457
			print("adjust elevator angle to: 3 deg")
		elif trajectory_angle >= 49.82:
			vehicle.channels.overrides['2'] = 1521
			print("adjust elevator angle to: 0 deg")
		elif trajectory_angle >= 45.51:
			vehicle.channels.overrides['2'] = 1580
			print("adjust elevator angle to: -5 deg")
		elif trajectory_angle >= 40.33:
			vehicle.channels.overrides['2'] = 1660
			print("adjust elevator angle to: -10 deg")
		elif trajectory_angle >= 35.11:
			vehicle.channels.overrides['2'] = 1720
			print("adjust elevator angle to: -15 deg")
		elif trajectory_angle >= 28.86:
			vehicle.channels.overrides['2'] = 1792
...
try:
	...
	#video_filename = "../../../Videos/flight/12-10-64_sikan_test_" + args.number_of_run + ".avi"
	video_filename = "../../../Videos/flight/19-10-64_sikan_deepstall_alt_test_" + args.number_of_run + ".avi"
